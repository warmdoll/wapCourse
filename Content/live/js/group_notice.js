...
function onApplyJoinGroupRequestNotify(notify) {
    WebBigGroupIM.Log.warn("执行 加群申请 回调："+JSON.stringify(notify));
...
function onApplyJoinGroupAcceptNotify(notify) {
    WebBigGroupIM.Log.warn("执行 申请加群被同意 回调："+JSON.stringify(notify));
...
function onApplyJoinGroupRefuseNotify(notify) {
    WebBigGroupIM.Log.warn("执行 申请加群被拒绝 回调："+JSON.stringify(notify));
...
function onKickedGroupNotify(notify) {
    WebBigGroupIM.Log.warn("执行 被踢出群  回调："+JSON.stringify(notify));
...
function onDestoryGroupNotify(notify) {
    WebBigGroupIM.Log.warn("执行 解散群 回调："+JSON.stringify(notify));
...
function onCreateGroupNotify(notify) {
    WebBigGroupIM.Log.warn("执行 创建群 回调："+JSON.stringify(notify));
...
function onInvitedJoinGroupNotify(notify) {
    WebBigGroupIM.Log.warn("执行 被邀请加群  回调: "+JSON.stringify(notify));
...
function onQuitGroupNotify(notify) {
    WebBigGroupIM.Log.warn("执行 主动退群  回调： "+JSON.stringify(notify));
...
function onSetedGroupAdminNotify(notify) {
    WebBigGroupIM.Log.warn("执行 被设置为管理员  回调："+JSON.stringify(notify));
...
function onCanceledGroupAdminNotify(notify) {
    WebBigGroupIM.Log.warn("执行 被取消管理员 回调："+JSON.stringify(notify));
...
function onRevokeGroupNotify(notify) {
    WebBigGroupIM.Log.warn("执行 群被回收 回调："+JSON.stringify(notify));
...
function onCustomGroupNotify(notify) {
    WebBigGroupIM.Log.warn("执行 用户自定义系统消息 回调："+JSON.stringify(notify));
...
function onGroupInfoChangeNotify(groupInfo) {
    WebBigGroupIM.Log.warn("执行 群资料变化 回调： "+JSON.stringify(groupInfo));
    ...
    if (newName) {
        //更新群组列表的群名称
        //To do
        WebBigGroupIM.Log.warn("群id="+groupId+"的新名称为："+newName);
    }
}

//显示一条群组系统消息
function showGroupSystemMsg(type, typeCh, group_id, group_name, msg_content, msg_time) {
    var sysMsgStr="收到一条群系统消息: type="+type+", typeCh="+typeCh+",群ID="+group_id+", 群名称="+group_name+", 内容="+msg_content+", 时间="+WebBigGroupIM.Tool.formatTimeStamp(msg_time);
    WebBigGroupIM.Log.warn(sysMsgStr);
    alert(sysMsgStr);
}